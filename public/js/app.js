...
var datos = [];
var compruebaDatos;
fetch('https://pokeapi.co/api/v2/pokemon?limit=2000').then(function (res) {
  return res.json();
}).then(function (res) {
  for (var i = 0; i < res.results.length; i++) {
    fetch(res.results[i].url).then(function (res) {
      return res.json();
    }).then(function (res) {
      return datos.push({
        res: res
...
function ejecutarDT(dato) {
  ...
  var table = new DataTable('#pokemons', {
    data: obj,
    ...
    columns: [{
      ...
      "render": function render(data, type, row, meta) {
        var imgSrc = row[1].res.sprites.front_default;
        return '<img style="width: 75px" src="' + imgSrc + '">';
      }
    }, {
      "render": function render(data, type, row, meta) {
        var nPokemon = row[1].res.id;
